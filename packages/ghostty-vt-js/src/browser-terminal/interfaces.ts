...
//   https://github.com/coder/ghostty-web/blob/6a1a50df5b4f6b34d1b1de10fad3a0fc811bfbc0/lib/interfaces.ts
...
export type TerminalRendererPreference = 'canvas' | 'gpu' | 'auto';
export type TerminalRendererBackend = 'canvas' | 'webgl2' | 'webgpu';
export const DEFAULT_RENDERER_PREFERENCE = 'auto' as const satisfies TerminalRendererPreference;
...
export interface RendererFrameTiming {
  /** Actual backend that painted the frame, after fallback selection. */
  backend: TerminalRendererBackend;
  /** CPU time spent in the scheduler's renderer path. */
  cpuMs: number;
  /** Optional backend detail; populated when GPU timer support lands. */
  drawMs?: number;
  uploadMs?: number;
}

/** Optional instrumentation around the scheduler-owned renderer call. */
export interface RendererTimingHooks {
  /** Runs inside the measured interval, immediately before render(). */
  beforeRender?: () => void;
  /** Runs after timing is captured, so reporting overhead is excluded. */
  onFrame: (sample: RendererFrameTiming) => void;
}
...
export interface ITerminalOptions {
  ...
  /** Paint backend preference. GPU/auto may fall back; default is auto. */
  renderer?: TerminalRendererPreference;
  /** Scheduler-owned per-frame renderer timing instrumentation. */
  rendererTiming?: RendererTimingHooks;
   ...
   * per side. Default 4. Subtracted before the cell division and combined with
   * the sub-cell leftover, so the grid ends up centred — do NOT use CSS padding
   * on the host for this (it can silently cost a row).
   */
  padding?: number;
  /**
   * Cell height as a multiple of the font's ascent+descent. Default 1.2.
   * Replaces the old hardcoded `+2px` per row (the geometry ownership design).
   */
  lineHeight?: number;
...
export interface ITerminalCore {
  ...
  readonly onRendererFailure: IEvent<RendererFailure>;
}
