...
//   https://github.com/coder/ghostty-web/blob/6a1a50df5b4f6b34d1b1de10fad3a0fc811bfbc0/lib/selection-manager.ts
...
import type { GhosttyTerminal } from './ghostty.js';
...
export class SelectionManager {
  ...
  /**
   * Convert surface pixels to a cell via the geometry authority — the mouse
   * has coordinates, the grid model knows where cells are. Local division by
   * cell size would ignore the grid's inset on the surface.
   */
  private pixelToCell(x: number, y: number): { col: number; row: number } {
    return this.terminal.geometryAuthority.cellAt(x, y);
  }
