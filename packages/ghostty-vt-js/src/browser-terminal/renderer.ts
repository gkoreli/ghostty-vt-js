...
//   https://github.com/coder/ghostty-web/blob/6a1a50df5b4f6b34d1b1de10fad3a0fc811bfbc0/lib/renderer.ts
// Substantially rewritten since; this file is ours and does not track that project.

import type { CellMetrics, TerminalGeometry } from './geometry.js';
/**
...
import type { GhosttyCell, ILink } from './types.js';
import { CellFlags } from './types.js';

// Interface for objects that can be rendered
export interface IRenderable {
  /**
   * Begin a render frame: re-snapshots the render state so subsequent
   * reads in this method (cursor, colors, lines, row-dirty flags) are
   * coherent with each other. Mirrors
   * `ghostty_render_state_update(state, terminal)` from `render.h`.
   *
   * Optional for backwards compatibility with consumers that don't speak
   * the frame contract; renderers should call it when present.
   */
  beginFrame?(): void;
  /**
   * End a render frame: clears global + per-row dirty flags. Mirrors
   * `ghostty_render_state_set(state, DIRTY, &NONE)` + the per-row clear walk.
   */
  endFrame?(): void;
  ...
  /**
   * @deprecated Prefer `endFrame()`. Kept so non-frame consumers can still
   * clear dirty flags explicitly.
   */
  clearDirty(): void;
  /**
...
export interface RendererOptions {
  /**
   * The terminal's geometry owner (the geometry-authority design). The renderer does NOT measure the
   * font or size the canvas on its own — it reads cell metrics and the canvas
   * box from here, so there is exactly one conversion from font → cells →
   * pixels in the system.
   */
  ...
  /** Receives an actionable backend failure after initial selection. */
...
/**
 * Default font stack: locally installed Nerd Fonts first, generic monospace
 * last.
 *
 * Terminal content routinely contains Private Use Area glyphs — eza/lsd file
 * icons, powerline prompt separators (p10k, starship), devicons — that
 * generic `monospace` cannot shape; they render as tofu. Canvas `fillText`
 * falls back through the stack PER GLYPH, so listing Nerd Fonts costs nothing
 * when absent and lights up icons when any one of them is installed — which
 * on a developer box running those tools is nearly always. Cell metrics are
 * measured with the same stack (geometry.ts), so measurement and paint can't
 * disagree. Consumers with a bundled webfont can still override via
 * `fontFamily`.
 */
export const DEFAULT_FONT_FAMILY =
  "'MesloLGS NF', 'MesloLGS Nerd Font', 'JetBrainsMono Nerd Font', "
  + "'FiraCode Nerd Font', 'Hack Nerd Font', 'CaskaydiaCove Nerd Font', "
  + "'Symbols Nerd Font', ui-monospace, Menlo, Consolas, monospace";

export const DEFAULT_THEME: Required<ITheme> = {
  foreground: '#d4d4d4',
  // Black, matching the wasm terminal's own built-in default. This constant is
  // pushed INTO the wasm terminal (one authority for 'default background', so
  // grid and gutter can never disagree) — keeping it identical to what the
  // engine would choose alone means adopting the resolved default is not a
  // visual change for anyone.
  background: '#000000',
  cursor: '#ffffff',
  cursorAccent: '#000000',
...
/**
 * Parse a CSS color string (`#rgb`, `#rrggbb`, or `rgb(r, g, b)`) into 0–255
 * RGB channels. Used to compare cell colors against the theme background so we
 * can skip per-cell fills for cells matching the default. Returns `{0,0,0}` if
 * the input is unrecognized — defensive: theme strings are well-controlled.
 */
...
export class CanvasRenderer implements ITerminalRenderer {
  public readonly backend = 'canvas' as const;
  ...
  // Pre-parsed RGB form of `theme.background`, used to identify cells whose bg
  // matches the theme default — those cells skip their per-cell fillRect because
  // the line-level fill already covers them. Recomputed in `setTheme()`.
  private themeBackgroundRgb: { r: number; g: number; b: number };
  private devicePixelRatio: number;
  /** The geometry owner — sole source of cell metrics and the canvas box. */
  ...
  constructor(canvas: HTMLCanvasElement, options: RendererOptions) {
    ...
    this.themeBackgroundRgb = parseCssColorToRgb(this.theme.background);
  ...
  /** Cell metrics, read from the geometry owner (never measured here). */
  private get metrics(): CellMetrics {
  ...
  public resize(cols: number, rows: number): void {
    // The canvas box is computed once, by the geometry owner (the geometry-authority design): CSS px
    // for layout, integer device px for the backing store. `cols`/`rows` are
    // accepted for the dirty-tracking comparison below but the box is NOT
    // re-derived here — that duplicate derivation is what let the canvas and
    // the grid disagree.
    const box = this.geometry.canvasBox();

    // The canvas covers the WHOLE pane; the grid is painted inset within it
    // (box.gridOrigin*). Consumers just size the host 100%/100% — there is no
    // CSS margin to manage and no second background to keep in sync.
    this.canvas.style.width = `${box.cssWidth}px`;
    this.canvas.style.height = `${box.cssHeight}px`;

    // Backing store in integer device pixels (fractional values are truncated
    // by the browser, which shifts every cell after the truncation).
    this.canvas.width = box.deviceWidth;
    this.canvas.height = box.deviceHeight;

    // Base transform = dpr scale + grid origin, applied ONCE here. Every cell
    // draw call below stays in grid space (x * cellWidth) and lands in the
    // right place; nothing else in this class knows the inset exists.
    // (Setting canvas.width/height reset the context, so this is authoritative.)
    this.ctx.setTransform(
      this.devicePixelRatio, 0, 0, this.devicePixelRatio,
      Math.round(box.gridOriginX * this.devicePixelRatio),
      Math.round(box.gridOriginY * this.devicePixelRatio),
    );
    ...
    // Fill the whole surface (including the inset gutter) with the theme
    // background — in surface space, so temporarily drop the grid transform.
    this.ctx.save();
  ...
  public render(
    ...
    // Frame contract: explicit `beginFrame()` re-snapshots the render state so
    // every read below this point (cursor, dims, dirty flags, lines) sees the
    // same point in time. Mirrors `ghostty_render_state_update(state, term)`
    // (`render.h`). Falls back to a no-op for legacy IRenderable shapes.
    buffer.beginFrame?.();
    ...
    // Resize if the backing store disagrees with the geometry owner's box.
    // Compared against canvasBox() — NOT re-derived from the grid — because the
    // surface covers the whole pane, not just the grid (a grid-based formula
    // here would force a resize on every frame and fight the inset design).
    const box = this.geometry.canvasBox();
    const needsResize =
      this.canvas.width !== box.deviceWidth || this.canvas.height !== box.deviceHeight;
    ...
    // Field comparison, not JSON.stringify — this runs at 60fps and
    // stringifying two objects per frame is pure allocation churn.
    ...
    // Frame contract: `endFrame()` clears global + per-row dirty flags. Even
    // after a full redraw — without this the next snapshot would still report
    // FULL/dirty rows and we'd never trip the incremental fast path.
    if (buffer.endFrame) {
      buffer.endFrame();
    } else {
      buffer.clearDirty();
   ...
   */
  /**
   * Run `draw` with the context clipped to the grid rect (grid space).
   *
   * Glyphs legitimately overhang their cells (antialiasing, italic overhang,
   * tall scripts). Inside the grid the adjacent-row redraw convention repaints
   * that spill, but at the grid's outer edge there is no adjacent row — the
   * spill lands in the gutter where nothing ever repaints it: permanent ghost
   * specks along the inset boundary. The old grid-sized canvas clipped these
   * by construction; the pane-sized surface must do it explicitly.
   *
   * Surface-space drawing (background fill, the scrollbar, which lives in the
   * gutter) must run OUTSIDE this bracket or it would be clipped away.
   */
  private withGridClip(dims: { cols: number; rows: number }, draw: () => void): void {
    this.ctx.save();
    this.ctx.beginPath();
    this.ctx.rect(0, 0, dims.cols * this.metrics.width, dims.rows * this.metrics.height);
    this.ctx.clip();
    try {
      draw();
    } finally {
  ...
  private renderLine(line: GhosttyCell[], y: number, cols: number): void {
    ...
    // Fill the line with the theme background — source-over, NO clearRect.
    // Line rects are fractional (cell height × row), so both clearRect and
    // fillRect antialias their edges. clearRect PARTIALLY clears the boundary
    // pixel row (alpha < 1) and the following fill repaints it with partial
    // alpha: partial × partial never returns to opaque, leaving a translucent
    // seam at every line boundary — invisible on a dark page, glaring the
    // moment the element behind the canvas has a color. The theme background
    // is opaque and the surface was filled opaque at resize, so a plain
    // composite fully covers; nothing here may punch alpha out of the surface.
    this.ctx.fillStyle = this.theme.background;
  ...
  private renderCellBackground(cell: GhosttyCell, x: number, y: number): void {
    ...
    // Skip the per-cell fill when the cell already matches the theme bg —
    // the line-level theme background drawn earlier covers it. We compare
    // against the parsed theme bg (`themeBgRgb`) rather than hardcoded
    // `0,0,0`, so this works for both light and dark themes.
    const themeBgRgb = this.themeBackgroundRgb;
    const isDefaultBg = bg_r === themeBgRgb.r && bg_g === themeBgRgb.g && bg_b === themeBgRgb.b;
    if (!isDefaultBg) {
  ...
  private renderScrollbar(
    ...
    const ctx = this.ctx;
    // The scrollbar belongs to the SURFACE (it hugs the pane's right edge),
    // not to the grid — draw it in surface space, outside the grid transform.
    ctx.save();
    ...
    // Reset the strip with an opaque background fill (covers the faded thumb
    // without clearRect — punching alpha leaves translucent seams at the
    // strip's antialiased edges, same defect as the per-line path).
   ...
   */
  public clear(): void {
    // Whole-surface operation → surface space, not grid space.
    this.ctx.save();
    this.ctx.setTransform(1, 0, 0, 1, 0, 0);
